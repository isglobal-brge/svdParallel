---
title: "Parallel functions for Singular Value Decomposition"
author: 
...
references:
- id: ref
  title: A distributed and incremental svd algorithm for agglomerative dataanalysis on large networks
  author: 
   - family: Iwen
     given: Mark
   - family: Ong
...
knitr::opts_chunk$set(collapse = TRUE, comment = "", 
                      cache=TRUE, message = FALSE, fig.height=4, fig.width=4, fig.align= "center"
)
```
...
Then, you can install `svdParallel` from Github by typing and then load it as usual.


```{r load_svdParallel, include=FALSE }
...
```{r,echo=TRUE, fig.width=7, fig.height=4,fig.show='hold',fig.align='center'}
par(mfrow=c(1,2))
...
```{r breast3, fig.width=7, fig.height=4,fig.show='hold'}
par(mfrow=c(1,2))
plot(gpcaL)
plot(gpcaL, type="individuals")
```

And we can also classify them according to the diagnosis of the tumor: benign or malignant. 
...
```{r breastTCGA5, fig.height=5, fig.width=5, fig.align= "center"}
group<-as.factor(breastTCGA$ER.Status)
