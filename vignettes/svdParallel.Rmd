...
```{r breastTCGA7, fig.show='hold', fig.align='center', fig.cap="Boxplot of the computational times for different functions. The first two are using Block method and the following two are using General Block method. The 5th is unsing PCA function and the last one is using ord function."}
mb <- microbenchmark::microbenchmark("Block method (k=2)"=getPCA(t(genes), parts = 2, center=F, scale=F),
                               ...
                               "General Block method (k=2, q=2)"=getPCA(t(genes), method="generalBlockSVD", k=2, q=2, center=F, scale=F),
                               "General Block method (k=3, q=2)"=getPCA(t(genes), method="generalBlockSVD", k=3, q=2, center=F, scale=F),
                               "PCA function"=PCA(t(genes), graph=FALSE), 
                               "ord function" = ord(genes, classvec=group, type = "pca"), times=10)
boxplot(mb, xaxt="n", xlab="")
axis(1, labels=c("Block k=2", "Block k=5", "k=3 q=2", "k=3 q=2", "PCA", "ord"), at =1:6,las=2)
```
...
```{r breastTCGA8, fig.show='hold', fig.align='center', fig.cap="Boxplot of the computational times for different functions. The first two are using Block method and the following two are using General Block method. The last one is using PCA0, it does not use any method."}
mb2 <- microbenchmark::microbenchmark("Block method (k=2)"=getPCA(t(genes), parts = 2, center=F, scale=F),
                               "Block method (k=5)"=getPCA(t(genes), parts = 5, center=F, scale=F),
                               "getPCA22"=getPCA(t(genes), method="generalBlockSVD", k=2, q=2, center=F, scale=F),
                               "General Block method (k=3, q=2)"=getPCA(t(genes), method="generalBlockSVD", k=3, q=2, center=F, scale=F),
                               "PCA0 no method"=getPCA0(t(genes), center=F, scale=F),  times=10)
boxplot(mb2, xaxt="n", xlab="")
axis(1, labels=c("Block k=2", "Block k=5", "k=2 q=2","k=3 q=2", "PCA0"), at =1:5,las=2)
```
